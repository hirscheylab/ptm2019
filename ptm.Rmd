...
##Note (lysine)
In this code chunk, the goal is to count and summarize lysine modifications. Counted 57 (as of Feb 2019), however does not include 3 published modifiations: succination, itaconylation, or s-acetylation, so OK to conclude 60, at least.
```{r}
ptm %>% 
  filter(!FT == "CROSSLNK") %>% #omit AA cross links only
  filter(TG == "Cysteine") %>% 
