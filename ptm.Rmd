...
ptm %>%
  filter(!FT == "CROSSLNK") %>% #omit AA cross links only
  ggplot() +
  geom_bar(aes(fct_rev(fct_infreq(TG, ordered = TRUE)), fill = fct_rev(fct_infreq(fct_lump(KW, 10)))), color = "black") +
  coord_flip() +
  labs(x = "", y = "Count") +
  expand_limits(y = 80) +
...
ptm_vec <- ptm %>%
  ...
  separate(id, into = c("id"), sep = "\\([:alpha:]{3}\\)$") %>% 
  distinct(id) %>% 
  pull(id)
...
ggplot(match1) +
  ...
  scale_fill_viridis(discrete = TRUE, direction = 1, option = "inferno", name = "Acyl-CoA Species", labels = c("Not Matching Lysine Modification", "Matching Lysine Modification")) +
  NULL
